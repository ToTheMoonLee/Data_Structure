 ...
 */
public class $811SubdomainVisitCount {

    public static void main(String[] args) {
        PrintUtil.printList(subdomainVisits(new String[]{"9001 discuss.leetcode.com"}));
    }
    ...
    static List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String s : cpdomains) {
            ...
            for (int j = 0; j < domains.length(); j++) {
                if (domains.charAt(j) == '.') {
                    String ss = domains.substring(j + 1, domains.length());
                    map.put(ss, map.getOrDefault(ss, 0) + Integer.valueOf(arr[0]));
                }
            }
            map.put(domains, map.getOrDefault(domains, 0) + Integer.valueOf(arr[0]));
        }
