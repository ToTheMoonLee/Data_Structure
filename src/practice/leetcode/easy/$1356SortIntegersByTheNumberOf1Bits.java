...
public class $1356SortIntegersByTheNumberOf1Bits {
    ...
    /**
     * their solutions of this problem
     * https://leetcode.com/problems/sort-integers-by-the-number-of-1-bits/discuss/?currentPage=1&orderBy=most_votes&query=
     */
    ...
    static int[] sortByBits(int[] arr) {
        ArrayList[] temp = new ArrayList[33];
        for (int i=0;i<temp.length;i++) {
            temp[i] = new ArrayList();
        }
        for (int i=0;i<arr.length;i++) {
            temp[countOne(arr[i])].add(arr[i]);
        }
        int[] ans = new int[arr.length];
        int i=0;
        for (ArrayList list : temp) {
